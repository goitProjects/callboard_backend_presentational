...
node("nodejs"){
    ...
    stage('Clone Git Repo') {
        catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
            git branch: 'main', credentialsId: 'github-goitProjects', url: 'git@github.com:goitProjects/callboard_backend_presentational.git'
        }
    }
    ...
    stage('Copy files'){
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                ...
                sh "mkdir -p /root/slave_jenkins/workspace/Build-Callboard-Backend-Presentational/src/gcp-config"
                //Copy files for Build stage
                sh '''cp /root/keys/index.ts /root/slave_jenkins/workspace/Build-Callboard-Backend-Presentational/src/gcp-config/index.ts'''
                sh "cp /root/keys/keys.json /root/slave_jenkins/workspace/Build-Callboard-Backend-Presentational/src/gcp-config/keys.json"
                sh "cp /root/keys/callboard-present/.env /root/slave_jenkins/workspace/Build-Callboard-Backend-Presentational/.env"
            }
        }
    }
    ...
    stage('Deploy') {
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                //sent files to  callboard-back-presentational.goit.global
                sh "scp -r ./* ${env.sshUserAndHost}:/root/callboard_backend_presentational"
            }
        }
    }
