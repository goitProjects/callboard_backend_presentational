...
node("nodejs"){
    ...
    stage('Copy files'){
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                sh "mkdir -p /root/slave_jenkins/workspace/Build-Callboard-Pl-Backend/src/gcp-config"
                sh '''cp /root/keys/index.ts /root/slave_jenkins/workspace/Build-Callboard-Pl-Backend/src/gcp-config/index.ts'''
                sh "cp /root/keys/keys.json /root/slave_jenkins/workspace/Build-Callboard-Pl-Backend/src/gcp-config/keys.json"
                sh "cp /root/keys/callboard-pl/.env /root/slave_jenkins/workspace/Build-Callboard-Pl-Backend/.env"
            }
        }
    }
    ...
    stage('Deploy') {
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                //sent files to  callboard-backend-pl.goit.global
                sh "scp -r ./* ${env.sshUserAndHost}:/root/callboard-backend-pl"
            }
        }
    }
