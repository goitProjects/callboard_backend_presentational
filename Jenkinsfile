...
node("nodejs"){
    ...
    stage('Copy files'){
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                sh "mkdir -p /root/slave_jenkins/workspace/Build-Callboard-En-Backend/src/gcp-config"
    ...
    stage('Deploy') {
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                //sent files to  callboard-backend-en.goit.global
                sh "scp -r ./* ${env.sshUserAndHost}:/root/callboard-backend-en"
            }
        }
    }
